...
class AuthManager {
    ...
    login(username, password, userType) {
        ...
        if (!isValid) {
            ...
            const maxAttempts = this.dataManager?.getSettings?.().maxLoginAttempts || this.maxLoginAttempts;

            let newLockedUntil = null;
            if (nextFail >= maxAttempts) {
                newLockedUntil = new Date(Date.now() + this.lockoutDuration).toISOString();
            }
