...
class AuthManager {
    ...
    hashPassword(password, salt) {
        ...
        // 注意：不取绝对值，保持与 DataManager.hashPassword 完全一致
    ...
    forgotPassword(username, userType, email) {
        ...
        // 实际应用中这里应该发送邮件
        console.log(`密码重置链接: http://localhost:3000/reset-password?token=${resetToken}`);
        ...
        for (const [userId, reset] of Object.entries(resets)) {
            if (reset.token === token && Date.now() < reset.expires) {
                ...
                if (user) {
                    ...
                    // 更新用户密码（这里应该调用数据管理器）
                    this.updateUserPassword(userId, hashedPassword, salt);
    ...
    changePassword(oldPassword, newPassword) {
        ...
        // 验证旧密码
        if (!this.verifyPassword(oldPassword, this.currentUser.password, this.currentUser.salt)) {
        ...
        this.updateUserPassword(this.currentUser.id, hashedPassword, salt);
