...
class DataManager {
        ...
        return {
            users: [
                ...
                },
                {
                    id: 's002',
                    username: '2021001002',
                    password: this.hashPassword('2021001002123', '2021001002'),
                    salt: '2021001002',
                    name: '李四',
                    userType: 'student',
                ...
                },
                {
                    id: 's003',
                    username: '2021001003',
                    password: this.hashPassword('2021001003123', '2021001003'),
                    salt: '2021001003',
                    name: '王五',
                    userType: 'student',
                ...
                },
                {
                    id: 't002',
                    username: 'teacher002',
                    password: this.hashPassword('teacher002123', 'teacher002'),
                    salt: 'teacher002',
                    name: '李教授',
                    userType: 'teacher',
                ...
                },
                {
                    id: 't003',
                    username: 'teacher003',
                    password: this.hashPassword('teacher003123', 'teacher003'),
                    salt: 'teacher003',
                    name: '张教授',
                    userType: 'teacher',
                ...
                // 系统管理员
                {
                    id: 'sys001',
                    username: 'sysadmin',
                    password: this.hashPassword('sysadmin123', 'sysadmin'),
                    salt: 'sysadmin',
                    name: '张系统',
                    userType: 'systemAdmin',
            ...
            courses: [
                {
                    ...
                    requirements: '已修完程序设计基础、具备C/C++基础',
                    extra: {
                        carouselImages: [
                            'https://picsum.photos/seed/course-ds-1/1200/400',
                            'https://picsum.photos/seed/course-ds-2/1200/400'
                        ],
                        galleryImages: [
                            'https://picsum.photos/seed/course-ds-3/900/520',
                            'https://picsum.photos/seed/course-ds-4/900/520'
                        ],
                        enableComments: true,
                        enableNotes: true,
                        highlights: ['算法复杂度分析', '数据结构实战项目']
                    },
                    maxStudents: 50,
                    currentStudents: 38,
                    category: 'required',
                ...
                },
                {
                    ...
                    requirements: '已修完程序设计基础',
                    extra: {
                        carouselImages: [
                            'https://picsum.photos/seed/course-net-1/1200/400'
                        ],
                        galleryImages: [],
                        enableComments: false,
                        enableNotes: true,
                        highlights: ['TCP/IP协议栈', '网络抓包分析']
                    },
                    maxStudents: 40,
                    currentStudents: 1,
                    category: 'required',
                ...
                },
                {
                    ...
                    requirements: '已修完数据结构',
                    extra: {
                        carouselImages: [],
                        galleryImages: [
                            'https://picsum.photos/seed/course-os-1/900/520'
                        ],
                        enableComments: true,
                        enableNotes: false,
                        highlights: ['进程调度模拟', '内存管理实验']
                    },
                    maxStudents: 45,
                    currentStudents: 28,
                    category: 'required',
                ...
                },
                {
                    ...
                    extra: {
                        carouselImages: [
                            'https://picsum.photos/seed/course-db-1/1200/400'
                        ],
                        galleryImages: [],
                        enableComments: true,
                        enableNotes: true,
                        highlights: ['SQL实训', '事务与并发控制']
                    },
                    maxStudents: 40,
                    currentStudents: 25,
                    category: 'elective',
                ...
                },
                {
                    ...
                    requirements: '已修完数据结构与算法',
                    extra: {
                        carouselImages: [],
                        galleryImages: [],
                        enableComments: false,
                        enableNotes: true,
                        highlights: ['动态规划专题', '图算法专题']
                    },
                    maxStudents: 35,
                    currentStudents: 20,
                    category: 'elective',
    ...
    ensureDataCompatibility() {
        ...
        this.data.users.forEach(user => {
            if (typeof user.requirePasswordChange === 'undefined') user.requirePasswordChange = false;
            if (!user.status) user.status = 'active';
            // 登录安全字段
            if (typeof user.failedLoginCount === 'undefined') user.failedLoginCount = 0;
            if (typeof user.lockedUntil === 'undefined') user.lockedUntil = null;
            if (typeof user.lastLoginAt === 'undefined') user.lastLoginAt = null;
        ...
        this.data.courses.forEach(course => {
            if (!course.requirements) course.requirements = '无特殊要求';
            if (!course.extra) {
                course.extra = {
                    carouselImages: [],
                    galleryImages: [],
            ...
            if (typeof course.extra.enableComments !== 'boolean') course.extra.enableComments = false;
            if (typeof course.extra.enableNotes !== 'boolean') course.extra.enableNotes = true;
            if (!Array.isArray(course.extra.carouselImages)) course.extra.carouselImages = [];
            if (!Array.isArray(course.extra.galleryImages)) course.extra.galleryImages = [];
            if (!Array.isArray(course.extra.highlights)) course.extra.highlights = [];
        });
    ...
    deleteCourse(courseId) {
        ...
        this.data.enrollments = (this.data.enrollments || []).filter(e => e.courseId !== courseId);
        this.data.assignments = (this.data.assignments || []).filter(a => a.courseId !== courseId);
        this.data.materials = (this.data.materials || []).filter(m => m.courseId !== courseId);
        this.saveData();
        return this.data.courses.length !== before;
    }
    ...
    // 更新登录失败次数/锁定时间
    updateUserLoginSecurity(userId, options = {}) {
        const userIndex = this.data.users.findIndex(u => u.id === userId);
        if (userIndex === -1) return false;
        const user = this.data.users[userIndex];
        const { failedLoginCount, lockedUntil, lastLoginAt } = options;
        const hasFailed = Object.prototype.hasOwnProperty.call(options, 'failedLoginCount');
        const hasLocked = Object.prototype.hasOwnProperty.call(options, 'lockedUntil');
        const hasLast = Object.prototype.hasOwnProperty.call(options, 'lastLoginAt');
        if (hasFailed) user.failedLoginCount = failedLoginCount;
        if (hasLocked) user.lockedUntil = lockedUntil;
        if (hasLast) user.lastLoginAt = lastLoginAt;
        // 同步 status
    ...
    createUser(user) {
        ...
        const initialPassword = user.initialPassword || `${user.username}123`;
    ...
    importStudents(rows, options = {}) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const majorFromRow = String(row.major || row.专业 || '').trim();
            const statusText = String(row.status || row.状态 || '').trim();
            const status = statusText
                ? (statusText.includes('锁') ? 'locked' : (statusText.includes('未') ? 'inactive' : 'active'))
                : 'active';
            ...
            if (!cls) {
                cls = {
                    ...
                    grade: options.grade || '',
                    major: options.major || majorFromRow || '',
                    headTeacher: '',
                    studentCount: 0
                };
            ...
            const rowPassword = defaultPassword || `${studentNo}123`;
            const user = {
                id: this.generateId(),
                username: studentNo,
                name,
                ...
                major: options.major || majorFromRow || cls.major || '',
                initialPassword: rowPassword,
    ...
    getCourseGradeStats(courseId) {
        const grades = this.data.grades.filter(grade => {
            ...
            if (!grade.assignmentScores) return false;
