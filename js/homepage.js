...
function loadCourses() {
    const coursesData = (window.dataManager?.getData('courses') || [])
...
function createCourseCard(course) {
    ...
    const teacher = window.dataManager?.getUserById(course.teacherId);
    const department = getDepartmentName(course.departmentId);
    ...
    card.innerHTML = `
        ...
        <div class="course-info">
            <p><i class="fas fa-user"></i> ${teacher ? teacher.name : '未指定'}</p>
            <p><i class="fas fa-university"></i> ${department}</p>
            <p><i class="fas fa-coins"></i> ${course.credits} 学分</p>
            <p><i class="fas fa-users"></i> ${course.currentStudents}/${course.maxStudents} 人已选修</p>
            <p><i class="fas fa-clipboard-list"></i> 选课要求：${course.requirements || '无特殊要求'}</p>
        </div>
...
function loadFilteredCourses(searchTerm, department, credits, type) {
    const allCourses = (window.dataManager?.getData('courses') || [])
        .filter(course => course.status === 'published');
    ...
    const departmentId = departmentMap[department] || department;
    ...
    const filteredCourses = allCourses.filter(course => {
        ...
        const matchesDepartment = !departmentId || course.departmentId === departmentId;
        const matchesCredits = !credits || course.credits.toString() === credits;
        const matchesType = !type || course.category === type;
    ...
    if (coursesContainer) {
        ...
        if (filteredCourses.length === 0) {
            ...
            renderCourses(filteredCourses, 0, filteredCourses.length);
...
function handleForgotPasswordSubmit() {
    ...
    const auth = getAuthInstance();
    if (!auth) {
        showMessage('error', '操作失败', '认证服务初始化失败，请刷新页面重试');
        return;
    }

    const result = auth.forgotPassword(username, userType, email);
    if (!result.success) {
        showMessage('error', '发送失败', result.message || '请检查输入信息');
        return;
    }

    closeForgotPasswordModal();
    showMessage('success', '发送成功', result.message || '重置链接已发送');

    // 演示模式：直接弹出重置弹窗，并预填令牌
    setTimeout(() => {
        showResetPasswordModal(result.token);
    }, 600);
}
...
function handleResetPasswordSubmit() {
    ...
    if (newPassword !== confirmPassword) {
        showMessage('error', '表单错误', '两次输入的密码不一致');
        return;
    }
    ...
    if (!result.success) {
        showMessage('error', '重置失败', result.message || '重置链接无效或已过期');
        return;
    }

    closeResetPasswordModal();
    showMessage('success', '重置成功', '密码已重置，请使用新密码登录');
    // 自动弹回登录
    setTimeout(() => {
        showLoginModal();
    }, 800);
