...
class AcademicAdminDashboard {
    ...
    renderAudit() {
        ...
        const pendingGrades = this.gradesData.filter(g => g.status === 'pending');
        
        auditList.innerHTML = pendingGrades.map(grade => {
            const course = this.coursesData.find(c => c.id === grade.courseId);
            const student = dataManager.getUserById(grade.studentId);
            ...
            return `
                <div class="audit-item">
                    <div class="audit-header">
                        <h4>${student ? student.name : '未知学生'} - ${course ? course.courseName : '未知课程'}</h4>
                        <span class="pending-badge">待审核</span>
                    </div>
                    <div class="audit-info">
                        <p><i class="fas fa-id-card"></i> 学号: ${student ? student.username : '未知'}</p>
                        <p><i class="fas fa-chart-line"></i> 成绩: ${grade.totalScore}</p>
                        <p><i class="fas fa-graduation-cap"></i> GPA: ${grade.gpa}</p>
                        <p><i class="fas fa-calendar"></i> 学期: ${grade.semester}</p>
                        <p><i class="fas fa-clock"></i> 提交时间: ${new Date().toLocaleString()}</p>
                    </div>
                    <div class="audit-actions">
                        <button class="btn-success" onclick="academicAdmin.approveGrade('${grade.id}')">通过</button>
                        <button class="btn-danger" onclick="academicAdmin.rejectGrade('${grade.id}')">驳回</button>
                        <button class="btn-info" onclick="academicAdmin.viewGradeDetails('${grade.id}')">查看详情</button>
                    </div>
                </div>
            `;
        }).join('');
