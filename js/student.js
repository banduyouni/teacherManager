...
class StudentDashboard {
        ...
        modalContent.innerHTML = `
            <div class="course-detail">
                ...
                <div class="detail-section">
                    ...
                    <p><strong>课程亮点：</strong>${course.extra?.highlights?.join('、') || '暂无'}</p>
    ...
    viewAssignments(courseId) {
        ...
        const materials = dataManager.getData('materials').filter(m => m.courseId === courseId);
        const assignments = dataManager.getCourseAssignments(courseId);
                ...
                <div class="modal-body">
                        ...
                        ${materials.length ? materials.map(m => `
                            <div class="material-item">
                                <div class="material-info">
                                    <div>
                                        <h4>${m.title}</h4>
                                        <p>类型：${m.type} | 上传时间：${new Date(m.uploadTime).toLocaleString()}</p>
                                    </div>
                                </div>
                                <div class="material-actions">
                                    <button class="btn-sm btn-secondary" data-view="${m.id}">查看</button>
                                </div>
                            </div>
                    ...
                    <div class="task-section" style="margin-top:16px;">
                        ...
                        ${assignments.length ? assignments.map(a => {
                            const submission = dataManager.getStudentSubmissions(this.userData.id, a.id)[0];
                            const statusText = submission ? '已提交' : '未提交';
                            return `
                                <div class="material-item">
                                    <div class="material-info">
                                        <div>
                                            <h4>${a.title} (${a.type === 'exam' ? '考试' : '作业'})</h4>
                                            <p>截止时间：${new Date(a.endTime).toLocaleString()} | 状态：${statusText}</p>
                                        </div>
                                    </div>
                                    <div class="material-actions">
                                        ${submission ? '<span class="status-badge published">已提交</span>' : `<button class="btn-sm btn-primary" data-submit="${a.id}">提交</button>`}
                                    </div>
                                </div>
                            `;
                        }).join('') : '<p class="no-materials">暂无作业</p>'}
                    </div>
                </div>
            </div>
        `;
        ...
        modal.querySelectorAll('button[data-view]').forEach(btn => {
            btn.addEventListener('click', () => {
                showMessage('课件已打开（演示）', 'info');
            });
        });
        modal.querySelectorAll('button[data-submit]').forEach(btn => {
            btn.addEventListener('click', () => {
                const assignmentId = btn.dataset.submit;
                const content = prompt('请输入提交内容（演示）');
                if (!content) return;
    ...
    renderProfile() {
        ...
        if (majorEl) majorEl.textContent = `${user.major || ''} ${user.grade || ''}`.trim();
        if (studentNoEl) studentNoEl.textContent = user.username || '--';
